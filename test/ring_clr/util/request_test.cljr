(ns ring-clr.util.request-test
  (:require [clojure.test :refer [deftest is testing]]
            [ring-clr.util.request :refer [body-string character-encoding content-length content-type in-context?
                                           request-url set-context urlencoded-form?]]
            [ring-clr.util.io :refer [string-input-stream]])
...
(deftest test-body-string
  ...
  (testing "file body"
    (let [f (-> (Path/GetTempFileName) (FileInfo.))]
      (spit f "bar")
...
(deftest test-set-context
  (is (= (set-context {:uri "/foo/bar"} "/foo")
         {:uri "/foo/bar"
          :context "/foo"
          :path-info "/bar"}))
  (is (thrown? Exception (set-context {:uri "/foo/bar"} "/bar"))))
